...
function buildCharts(sample) {
  ...
  d3.json("samples.json").then((data) => {
    ...
    let sampleValues = resultDict.sample_values;
  
    //write a loop to add the OTU text to our array for plotting y values
   var list=[];
   for (var j=0; j< otuIds.length; j++) {
      list.push('OTU ' + otuIds[j])
    };

    //since our sample data is already sorted for all cases we can just slice the top 10
    // and reverse the order for plotting
